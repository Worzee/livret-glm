...
- **UI** (`CentreAlertes`) : rendu dédié pour ce type — icône drapeau, libellé
  cliquable (ouvre l'entretien pour le contexte) + case à cocher « traité » ;
  câblée dans `TableauDeBord` (gated par `point-alerte.traiter`). Les autres
  rôles ne voient pas ces alertes ; l'apprenti·e / maître gardent leur vue
