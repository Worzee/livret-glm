...
L'**étape 1 du CDC v1.3 est livrée et déployée**, enrichie par 8 vagues post-livraison (CDC v1.5 + chantiers métier mai 2026 + retours coordonnateurs juin 2026 + réunion direction 1ᵉʳ juillet 2026). La maquette est fonctionnelle, accessible sur URL publique avec Basic Auth, et tous les flux pédagogiques sont testés en bout-en-bout : **730 tests unitaires + 214 tests E2E passent**, bundle JS gzippé ~185 KB. Aucune authentification réelle ni backend persistant pour l'instant — c'est précisément l'objet de l'étape 2.
...
| État des tests (730 unit + 214 E2E)                             | §6                                                                                                        |
...
| **Tests unitaires**   | **730 / 730 ✓** (Vitest, 52 fichiers de test)                                                    |
| **Tests E2E**         | **214 / 214 ✓** (Playwright — Chromium desktop + mobile Pixel 5, 30 specs)                       |
...
| **`lib/alertes.test.ts`**                       | **21** | **Centre d'alertes par rôle : signatures attendues, verrouillages, initialisations, R7, affectations incomplètes (3 juillet 2026) + points d'alerte de l'entretien (8 juillet) + documents non attestés (10 juillet — formateur hors réservés) + documents obligatoires MANQUANTS coordo/admin (13 juillet)** |
...
| **`lib/documents-administratifs.test.ts`**      | **23** | **Documents nominatifs (10 juillet 2026 ; v2 13 juillet) : typologie (4 obligatoires + « autre », libellés), visibilité du flag « réservé », verrous de suppression et d'attestation (« lu et attesté »), types manquants + état des obligatoires, validation de dépôt (titre pour « autre » seul, réservé refusé hors « autre », type / taille)** |
...
└── src/
    ...
    │   └── useDocumentsStore.ts    # v2 — documents administratifs nominatifs (10 juillet ; typologie + attestation après lecture 13 juillet)
...
npm run e2e            # 214 tests E2E Playwright (build + preview + tests)
