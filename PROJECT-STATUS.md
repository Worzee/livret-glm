...
**Dernière mise à jour** : 2026-07-13 — **GEL DU FONCTIONNEL + lancement de la phase de déploiement o2switch (décision pilote, fin de journée)** : le périmètre fonctionnel de la maquette est **clos** — la demande 2 « entretiens individuels mi/fin de parcours » (cadrée le 2026-07-12) est **reportée après le déploiement** (trames officielles GRETA non reçues ; elle sera implémentée directement dans la stack cible étape 2) ; les **prérequis administratifs** sont lancés (DPO/AIPD mineurs en **chemin critique**, compte fonctionnel Nuage, App Registration Entra, ressources cPanel o2switch, Mailjet) — checklist, marqueurs du kit et séquence de bascule dans [`chantier-deploiement-o2switch.md`](chantier-deploiement-o2switch.md). Le même jour, plus tôt : **apprentis MINEURS : responsables légaux + 6ᵉ rôle (réunion pilote × directeur général — demande 5, arbitrages 2026-07-13)** : à l'inscription d'un·e apprenti·e **mineur·e** (minorité **recalculée en continu** depuis la date de naissance — arbitrage 1), saisie obligatoire de **1 à 2 responsables légaux** (prénom, nom, email obligatoires + téléphone, lien de parenté ; **emails différents** de l'apprenti·e, uniques SAUF **rattachement fratrie** : un email déjà connu comme responsable = même personne) — dans la **modale d'inscription manuelle** (section conditionnée à la minorité) ET dans l'**import Excel** (10 colonnes « resp. légal » ajoutées au modèle, **optionnelles à l'en-tête** — un fichier sans elles reste importable pour des majeurs — mais 1 responsable exigé par ligne mineure). Nouveau **6ᵉ rôle « Responsable légal »** (cyan, sélecteur de rôles + sélecteur de responsable actif Thi ↔ Duc, périmètre limité à ses enfants via `Apprenti.responsableLegalIds`) : il **ATTESTE les documents administratifs en lieu et place du mineur** (« lu et attesté » — sa consultation déverrouille, auteur·rice tracé·e dans l'attestation `attestePar*`, wording UI/PDF/alertes « attestation du responsable légal attendue », alertes « votre attestation est attendue » dans son centre d'alertes), **signe le slot « représentant légal » de l'entretien tripartite** (identité auto depuis son compte — le formateur garde le fallback) et consulte **tout le reste en LECTURE SEULE** (test transverse : balayage exhaustif de la matrice). L'**apprenti·e mineur·e** passe en **lecture sur les documents** (pas de bouton ni bandeau d'attestation) et garde son accès classique partout ailleurs ; à la **majorité**, il/elle récupère automatiquement l'attestation. Bumps **`livret-utilisateurs` v7** (Minh NGUYEN devient MINEUR, responsables Thi + Duc de démo) et **`livret-documents` v4** (auteur d'attestation ; protection des données de Minh non attestée). Nouvelles libs pures `minorite` (`estMineur`) et `responsables-legaux` (validation partagée modale/import). ⚠ **RGPD : la réintroduction des mineurs impose de réexaminer le dossier étape 2 avec le DPO** (AIPD — personnes vulnérables, cf. chantier). « À terme » (étape 2) : ces données créeront les comptes des responsables (tracé dans TODO-etape-2.md). Cadrage : [`chantier-demandes-direction-2026-07.md`](chantier-demandes-direction-2026-07.md) (demande 5). Le même jour, plus tôt : **documents au niveau FORMATION : dépôt en masse pour toute une promo (réunion pilote × directeur général — demande 4, arbitrages 2026-07-13)** : le coordo dépose un document générique (règlement intérieur, protection des données, droit à l'image, « Autre » — **tous les types SAUF le contrat pédagogique**, nominatif par nature, et jamais de flag « réservé ») **une seule fois pour toute la formation** depuis la page **Formations** (bouton « Documents » par carte — point d'entrée unique) : le document est **rattaché à la formation** (nouvelle entité `DocumentFormation`, stocké une fois — pas de copie), visible de tous les apprenti·e·s **actuels et futurs** de la promo, chacun·e **attestant individuellement après lecture** (consultations et attestations indexées par apprenti·e). **Le nominatif prime** à type égal ; un dépôt en masse d'un type obligatoire **fait tomber les anomalies « dépôt à effectuer »** de la promo ; **remplacement en masse** = toutes les attestations repartent de zéro (avertissement) ; **suppression verrouillée dès la première attestation** (esprit R21). La modale liste les documents de la promo (compteur « attesté par x/y », suppression) ; la page Documents fusionne nominatifs + formation (badge « Document de la formation », lib `documentsEffectifsApprenti`) — alertes, bandeaux, état des 4 obligatoires et PDF suivent. Bump **`livret-documents` v3** (fixtures : le règlement intérieur devient un document de formation CAP + BTS, attesté par tou·te·s sauf Sofia). Cadrage : [`chantier-demandes-direction-2026-07.md`](chantier-demandes-direction-2026-07.md) (demande 4). Le même jour, plus tôt : **documents administratifs v2 : typologie de 4 documents obligatoires + attestation sans signature (réunion pilote × directeur général — demande 3, arbitrages 2026-07-13)** : la liste déroulante du dépôt impose un **type** — Contrat pédagogique, Information relative à la protection des données, Droit à l'image, Accusé réception du règlement intérieur (**obligatoires**) ou « Autre document » (titre libre, **seul type acceptant le flag « réservé »**) ; **anomalie « dépôt à effectuer »** dans le centre d'alertes **coordo/admin** tant qu'un type obligatoire n'est pas déposé (type `document-manquant`) ; l'apprenti·e n'appose **plus de signature manuscrite** : **attestation simple horodatée** (confirmation à 2 clics, esprit R19/R21 — irréversible, document attesté insupprimable), possible **seulement après lecture** (bouton grisé tant que le document n'a pas été consulté — `consulteParApprentiLe`, tracé au clic « Consulter » de l'apprenti·e) ; **redéposer un type remplace** l'ancien document, même attesté (attestation remise à zéro, avertissement dans la modale) ; **bandeau d'état des 4 obligatoires** sur la page (attesté / attestation attendue / non déposé) et **page « Documents administratifs » du PDF toujours présente** avec l'état des 4 types + documents complémentaires. Bump **`livret-documents` v2** (fixtures : 6 dossiers complets attestés + cas de démo Léa — droit à l'image manquant, protection des données à attester, convention réservée — et Yanis — contrat pédagogique à attester, visible de son formateur). Cadrage : [`chantier-demandes-direction-2026-07.md`](chantier-demandes-direction-2026-07.md) (demande 3 — première d'une série issue de la réunion DG, recueil en cours). Vague précédente (2026-07-10) : **documents administratifs nominatifs avec attestation signée (demande direction, arbitrages pilote)** : nouveau menu « Documents administratifs » — la « partie 1 » du livret papier (engagements, convention, règlement…) se dépose **par apprenti·e** (documents nominatifs, coordo/admin, PDF/image ≤ 2 Mo), se consulte par tous les rôles **sauf** flag « réservé à l'apprenti·e » (→ apprenti·e + coordo + admin), et s'**atteste par signature manuscrite tactile obligatoire** de l'apprenti·e, document par document (suivi au centre d'alertes + bandeau apprenti·e + rappel dans le PDF du livret — page dédiée en tête, documents réservés sans titre). Nouveau store **`livret-documents`** (12ᵉ store), lib pure `documents-administratifs`, ressources `documents.gerer` / `documents.attester` (matrice 55 → 57). Maquette : fichiers en data-URL localStorage — **étape 2 : Nuage (apps.education.fr) via WebDAV** (cadrage : [`chantier-demandes-direction-2026-07.md`](chantier-demandes-direction-2026-07.md) ; demande 2 « entretiens individuels » cadrée le 2026-07-12 puis reportée post-déploiement o2switch). Le même jour, plus tôt : **mode de pédagogie affiché sur le tableau de bord (demande pilote)** : l'en-tête de chaque groupe de formation (vue groupée formateur / coordo / admin) porte un badge « **Pédagogie en Compétences** » ou « **Pédagogie en Activités** » (dérivé de `Formation.modeEvaluation`, même code couleur que le badge des pages d'administration). Le même jour, plus tôt : **bascule en mode activités à balayage partiel (retour démo direction — révision de l'arbitrage Q6 du chantier #4)** : le passage d'une formation en mode activités n'exige plus le **balayage complet** du référentiel — il est débloqué dès que **chaque activité du modèle fait appel à au moins une compétence évaluable** (les compétences non couvertes n'apparaissent simplement pas dans la Synthèse, la jauge de balayage devient informative). Gardes recalibrées sur le nouvel invariant : réactivation d'une compétence exclue **libre** (ancienne garde supprimée), **exclusion** bloquée si une activité tomberait à 0 compétence (nouvelle garde `peutExclureCompetence`), retouche du mapping refusée seulement si elle laisse une activité sans compétence ; réimport / changement de référentiel toujours bloqués en mode activités. Détail dans [`chantier-referentiels-competences.md`](chantier-referentiels-competences.md) § « Révision d'arbitrage (2026-07-10) ». Vague précédente (2026-07-08) : **remontée des points d'alerte de l'entretien dans « À traiter » du coordo / admin (demande pilote)** : les points d'alerte de la trame (réponses signalant une difficulté — « action à mener par le GRETA CFA ») d'un entretien **signé** remontent dans le centre d'alertes du coordo / admin, **marquables « traité » après confirmation** (« Confirmez-vous avoir réglé ce problème ? », bouton à 2 clics ; nouvelle ressource `point-alerte.traiter` ; état suivi dans `Livret.pointsAlerteTraites`, **hors de l'entretien** — l'entretien tripartite reste strictement inchangé ; champ optionnel, **aucun bump**). Nouvelle lib pure `points-alerte` (`pointsAlerteNonTraites`, remontée conditionnée à la signature 3/3). Le même jour, plus tôt : **fiabilisation des libellés des tableaux de bord (audit pilote)** : la carte « Alertes R7 » du bandeau de pilotage affichait « entretien 1 en retard » et le bandeau « Choix figé » de la sélection d'attitudes « l'entretien 1 est signé » — le « 1 » est périmé depuis l'entretien tripartite **unique** (6 juillet) ; remplacé par « entretien(s) tripartite(s) en retard » (accord au nombre d'alertes) et « l'entretien tripartite est signé ». Les KPI et le centre d'alertes eux-mêmes sont **exacts**, vérifiés par recoupement (bandeau global = somme des mini-stats par formation = cartes individuelles). Le même jour, plus tôt : **saisie détaillée de l'activité libre (hors modèle) en période entreprise (demande pilote)** : quand le maître / tuteur ou le formateur référent ajoute une « activité libre » sur une fiche de période, la colonne « Activité » présente désormais une **zone de texte éditable** (intitulé + contexte / détails) au lieu du libellé figé « Activité libre » — réutilise le champ `libelleLibre` existant (**aucun bump de schéma**), édition ouverte par la ressource `fiche.ajouter-competence` (formateur + maître / tuteur), valeur reprise telle quelle dans le PDF ; nouvelle lib pure `lignes-suivi` (`estLigneLibre`). Vague précédente (2026-07-07) : **auto-réparation des livrets manquants (bug pilote : un apprenti créé par l'utilisateur perdait son livret à chaque bump de `livret-donnees` — l'ouverture retombait sur « Aucun·e apprenti·e sélectionné·e » ; `AppShell` recrée désormais au montage un livret vierge avec planning hérité et sélections réalignées)**, après la **rubrique « Difficultés éventuelles » de l'entretien : polarité Oui/Non inversée (demande pilote — « Oui » = difficulté déclarée, en rouge avec icône d'alerte et repris dans le récapitulatif ; « Non » = norme, en vert ; consigne et PDF alignés, bump `livret-donnees` v26)**, après le **polish typographique global (demande pilote) : les tirets cadratins « — » des textes affichés sont remplacés par une ponctuation contextuelle (« : », parenthèses, « - », « … », virgule — détail §4)**, après le **chantier référentiels/compétences, modification #4 (décision pilote, 9 questions de cadrage arbitrées) : ÉVALUATION PAR ACTIVITÉS** — certaines formations s'évaluent par **activités** plutôt que par compétences (le référentiel reste impératif) : import de **modèles d'activités** (CSV/XLSX, activités seules), **mapping activités ↔ compétences dans l'UI** avec **jauge de balayage** (couverture complète du référentiel hors exclusions requise), **choix du mode d'évaluation par formation** (verrouillé dès la première saisie signée dans la promo, dans les deux sens), **fiches entreprise par activités** (sélection d'activités à l'entretien — mêmes règles que la sélection de compétences, activité libre autorisée, R20 adapté), **Synthèse toujours PAR COMPÉTENCES alimentée par la projection des activités** (last-write-wins chronologique, provenance « via activité X — Période N », écrasement manuel conservé), gardes du référentiel vivant (réimport / changement de référentiel / réactivation d'exclue bloqués en mode activités) ; **la promo CAP Cuisine passe en mode activités** (modèle de démo 6 activités, balayage 10/10) — arbitrages dans [`chantier-referentiels-competences.md`](chantier-referentiels-competences.md). Vague précédente (2026-07-06) : **chantier référentiels/compétences, modifications #1 à #3 (décisions pilote) : entretien tripartite UNIQUE et obligatoire (suppression des entretiens 2 à 4, du nombre d'entretiens par formation, de la banque de questions et de la modalité présentiel/distanciel — le suivi ultérieur passe par les fiches de suivi existantes), LIMITE DE 40 LIGNES ÉVALUABLES par référentiel à l'import (agrégation au niveau supérieur ou cochage manuel, exclusions conservées et gérables, seuil modifiable par l'admin seul), puis PÉRIODES EN CENTRE SIMPLIFIÉES + menu « SYNTHÈSE » (fiches centre réduites aux observations de fin de période — apprenti·e bloquante, formateur non ; attitudes professionnelles évaluées par le tuteur À CHAQUE période en entreprise, toutes exigées pour sa signature ; « Évaluation finale » renommée « Synthèse », restreinte aux compétences abordées en stage, colonne unique « Acquis en entreprise » — cadrage et arbitrages dans [`chantier-referentiels-competences.md`](chantier-referentiels-competences.md)).** Historique de la vague précédente (retours coordonnateurs pédagogiques — « Maître / Tuteur », affectation des questions par le coordo, jusqu'à 4 entretiens tripartites, motifs par rôle + séquencement, attitudes professionnelles par entretien + catalogue admin, confirmation avant écrasement d'une évaluation héritée, second maître / tuteur par apprenti·e, tri par année de formation sur le tableau de bord, **signature manuscrite tactile**, **répartition des apprenti·e·s entre coordos**, **questions de l'entretien gérées par formation**, **compétences activées par défaut + maître seul**, **bascule de périmètre coordo depuis l'administration**, **harmonisation du planning des périodes sur toute la promo**, **modalité présentiel/distanciel des entretiens + verrou de la fiche de suivi par signature**, **exports PDF par période / entretien / fiches de suivi**, **séquencement de visibilité des périodes**, **refonte de l'entretien tripartite 1 sur la trame officielle GRETA**, **ajout de compétences à la fiche de période ouvert au tuteur**, **fiche de période réduite à « Évaluation entreprise » + « Retour apprenti·e » (colonne GRETA CFA retirée)**, **périodes en centre de formation (miroir de l'entreprise)**, **récapitulatif personnel sur le tableau de bord apprenti·e**, **retrait des 4 attitudes redondantes avec l'appréciation maître (catalogue 16 → 12)**, **banque de questions réservée à l'admin**, **forçage de l'affichage des périodes par le coordo**, **import des référentiels CSV/XLSX mixtes 2/3 niveaux + affichage « libellé seul »**, **entreprise d'accueil gérée comme entité (choix par apprenti·e + traçabilité)**, **entreprise modifiable depuis les affectations**, **sélection de compétences réalignée « tout coché » au changement de référentiel + co-saisie des champs du tuteur par le formateur**, **entretien consultable en lecture seule avant initialisation**, **tableau de bord groupé par formation (sections repliables)**, **modale Planning épurée (formulaire d'ajout masqué, questions retirées)**, **retrait du suivi GRETA CFA de toutes les fiches (entreprise puis centre — tout se rédige dans les observations)**, **2 signatures en entreprise (apprenti·e + tuteur, le formateur commente et verrouille)**, **3 commentaires individuels sur l'entretien 1**, **grille de synthèse E1 colorée par niveau**, **récapitulatif des 4 attitudes obligatoires dans la synthèse des attitudes + PDF**, **en-tête des PDF scindé (Marianne à gauche, réseau GRETA CFA à droite, titre dessous)**, **PDF colorés (niveaux/appréciations/Oui-Non aux couleurs du site) + champs en ligne + tirets simples**, **2ᵉ promo de démo BTS MHR (référentiel 3 niveaux, 4 entretiens, 2ᵉ site)**, **tableau de pilotage coordo/admin (KPI par promo)**, **centre d'alertes par rôle**, **sélecteur de formateur actif**, **page « Accès mobile » (QR code, encadrement)**)
**Version applicative** : 0.1.0
**Phase CDC** : Étape 1 — maquette fonctionnelle (CDC v1.3) **livrée + vagues post-livraison successives** (dernière : réunion DG du 13 juillet 2026, demandes 3 à 5) — **fonctionnel gelé le 2026-07-13, phase de déploiement o2switch (étape 2) lancée**
...
L'**étape 1 du CDC v1.3 est livrée et déployée**, enrichie par les vagues post-livraison successives (CDC v1.5 + chantiers métier mai 2026 + retours coordonnateurs juin 2026 + réunion direction 1ᵉʳ juillet 2026 + chantiers référentiels et demandes direction juillet 2026 — dernière vague : réunion DG du 13 juillet 2026, demandes 3 à 5). La maquette est fonctionnelle, accessible sur URL publique avec Basic Auth, et tous les flux pédagogiques sont testés en bout-en-bout : **767 tests unitaires + 220 tests E2E passent**, bundle JS gzippé ~195 KB. Aucune authentification réelle ni backend persistant pour l'instant — c'est précisément l'objet de l'étape 2.
...
| **10 juillet 2026** (demande direction) | **Documents administratifs nominatifs** : menu « Documents administratifs » — dépôt par apprenti·e (coordo/admin, PDF/image ≤ 2 Mo), flag « réservé à l'apprenti·e » (consultation apprenti·e + coordo + admin seulement), **attestation par signature manuscrite tactile obligatoire** de l'apprenti·e par document, suivi (alertes + bandeau apprenti·e) et rappel dans le PDF (page dédiée). 12ᵉ store `livret-documents` ; étape 2 : binaires sur Nuage/WebDAV. Cadrage : [`chantier-demandes-direction-2026-07.md`](chantier-demandes-direction-2026-07.md). |
| **10 juillet 2026** (demande pilote) | **Mode de pédagogie sur le tableau de bord** : badge « Pédagogie en Compétences » / « Pédagogie en Activités » dans l'en-tête de chaque groupe de formation (vue groupée formateur / coordo / admin) — dérivé du mode d'évaluation de la formation (chantier #4). |
| **10 juillet 2026** (retour démo direction) | **Bascule en mode activités à balayage partiel** (révision de l'arbitrage Q6 du chantier #4) : la bascule est débloquée dès que **chaque activité fait appel à au moins une compétence** du référentiel — le balayage complet n'est plus exigé (jauge informative, compétences non couvertes absentes de la Synthèse). Réactivation d'exclue libre, exclusion gardée par `peutExclureCompetence` (activité à 0 compétence), mapping libre hors ce garde-fou. |
| **13 juillet 2026** (réunion DG — demande 3) | **Documents administratifs v2** : typologie imposée au dépôt — **4 types OBLIGATOIRES** (Contrat pédagogique, Information relative à la protection des données, Droit à l'image, Accusé réception du règlement intérieur) + « Autre document » (titre libre, seul type « réservable ») ; **anomalie « dépôt à effectuer »** (coordo/admin) par type manquant ; **attestation SANS signature manuscrite** (confirmation horodatée irréversible), possible **seulement après lecture** du document ; **remplacement par type** (attestation remise à zéro) ; **bandeau d'état des 4 obligatoires** sur la page et dans le **PDF** (page toujours présente). Bump `livret-documents` v2. Cadrage : [`chantier-demandes-direction-2026-07.md`](chantier-demandes-direction-2026-07.md). |
| **13 juillet 2026** (réunion DG — demande 4) | **Documents au niveau FORMATION (dépôt en masse)** : le coordo dépose une fois pour toute la promo depuis la page **Formations** (bouton « Documents » par carte, modale avec liste des documents de la promo — compteur « attesté par x/y », suppression tant que personne n'a attesté). **Tous les types sauf Contrat pédagogique** (nominatif par nature), pas de flag « réservé ». Document **rattaché à la formation** (`DocumentFormation`, apprenti·e·s actuels ET futurs), **attestation individuelle après lecture**, **le nominatif prime** à type égal, **remplacement en masse** = attestations de la promo remises à zéro. Fusion partout via `documentsEffectifsApprenti` (page, alertes, bandeaux, PDF — badge « Document de la formation »). Bump `livret-documents` v3. |
| **13 juillet 2026** (réunion DG — demande 5) | **Apprentis MINEURS : responsables légaux + 6ᵉ rôle** : inscription (modale + import Excel — 10 colonnes optionnelles au modèle) avec **1-2 responsables légaux obligatoires si mineur·e** (emails ≠ apprenti·e, **rattachement fratrie** par email connu), minorité **recalculée en continu**. Nouveau rôle **« Responsable légal »** (cyan) : **atteste les documents en lieu et place du mineur** (auteur tracé, « lu et attesté » via SA consultation), **signe le slot « représentant légal »** de l'entretien (identité auto), **lecture seule partout ailleurs** (balayage exhaustif de la matrice en test). L'apprenti·e mineur·e est en lecture sur les documents, accès classique ailleurs. Bumps `livret-utilisateurs` v7 (Minh mineur + Thi/Duc) et `livret-documents` v4. ⚠ RGPD : dossier étape 2 à réexaminer (mineurs = AIPD). |
| **8 juillet 2026** (demande pilote) | **Points d'alerte de l'entretien remontés au coordo / admin** : les réponses de la trame signalant une difficulté (« action à mener par le GRETA CFA ») d'un entretien **signé** apparaissent dans « À traiter » du tableau de bord coordo / admin, **cochables « traité »** (ressource `point-alerte.traiter`, suivi dans `Livret.pointsAlerteTraites` hors entretien). L'entretien reste inchangé ; lib pure `points-alerte` ; aucun bump. |
| **8 juillet 2026** (retour pilote) | **Activité libre détaillable en période entreprise** : l'ajout d'une « activité libre » (hors modèle en mode activités, hors référentiel en mode compétences) ouvre une **zone de texte éditable** dans la colonne « Activité » — le maître / tuteur ou le formateur référent y saisit l'**intitulé et le contexte / détails** de l'activité réalisée, au lieu du libellé figé « Activité libre ». Réutilise `libelleLibre` (aucun bump), lib pure `lignes-suivi` (`estLigneLibre`), PDF inchangé (affiche déjà `libelleLibre`). |
...
> **Phase de déploiement o2switch LANCÉE le 2026-07-13** (fonctionnel gelé ; demande 2
> reportée post-déploiement) : prérequis administratifs, marqueurs du kit, brouillon de
> note DPO et séquence de bascule suivis dans
> [`chantier-deploiement-o2switch.md`](chantier-deploiement-o2switch.md).
...
| État des tests (767 unit + 220 E2E)                             | §6                                                                                                        |
...
| **Tests unitaires**   | **767 / 767 ✓** (Vitest, 54 fichiers de test)                                                    |
| **Tests E2E**         | **220 / 220 ✓** (Playwright — Chromium desktop + mobile Pixel 5, 31 specs)                       |
...
- **Style** : Tailwind CSS 3 + shadcn/ui (tokens CSS variables, palette 6 rôles — équilibrage mai 2026, + responsable légal cyan juillet 2026)
- **State** : Zustand 5 + middleware `persist` — **12 stores** persistés en localStorage :
  - `livret-donnees` (schema v26) — livrets, fiches **entreprise** (compétences **ou activités selon le mode de la formation** + **évaluations des attitudes par période** — juillet 2026) **+ centre de formation** (`fichesSuiviCentre`, **observations de fin de période seules**), **l'entretien tripartite unique et obligatoire** (`Livret.entretien` — trame officielle GRETA), synthèse des compétences (colonne entreprise seule), sélection des compétences abordées en entreprise **+ sélection des activités prévues en entreprise** (mode activités — chantier #4), **forçage d'affichage des périodes** (`affichagePeriodesForce`), **suivi des points d'alerte de l'entretien traités par la coordination** (`pointsAlerteTraites` — hors entretien, champ optionnel, 8 juillet 2026)
  - `livret-role-actif` — rôle + maître / coordo / formateur actifs
  - `livret-apprenti-actif` — id de l'apprenti·e affiché·e
  - `livret-utilisateurs` (schema v7) — apprenti·e·s (avec **second maître / tuteur optionnel**, **coordo de rattachement**, **entreprise d'accueil par id + historique des affectations** juin 2026, **responsables légaux 1-2 si mineur·e** — 13 juillet 2026, demande 5), maîtres (avec `entreprise` + `fonction`), formateurs, coordos, admins, **responsables légaux** (rattachement fratrie par email)
  ...
  - `livret-documents` (schema v4) — **documents administratifs** (10 juillet 2026, demande direction ; **v2/v3/v4 le 13 juillet 2026 — réunion DG**) : **typologie de 4 types OBLIGATOIRES + « autre »** (seul type à titre libre et flag « réservé à l'apprenti·e »), fichiers en data-URL (≤ 2 Mo — étape 2 : Nuage/WebDAV), **attestation simple horodatée après lecture** (`consulteParApprentiLe`, sans signature manuscrite), **remplacement par type** (attestation remise à zéro) ; **v3 : documents au niveau FORMATION** (`documentsFormation` — dépôt en masse depuis la page Formations, attestations individuelles par apprenti·e, le nominatif prime) ; **v4 : auteur·rice d'attestation tracé·e** (`attestePar*` — apprenti·e majeur·e ou responsable légal d'un·e mineur·e, demande 5)
...
- **Suivi 8 juillet 2026 — fiabilisation des libellés (audit pilote)** : les compteurs (fiches signées, entretiens réalisés, alertes R7) et le centre d'alertes ont été audités et sont **exacts** (recoupement bandeau global / mini-stats par formation / cartes). Seuls deux libellés dataient de l'ère multi-entretiens : la carte « Alertes R7 » du bandeau (« entretien 1 en retard » → « entretien(s) tripartite(s) en retard », accordé au nombre) et le bandeau « Choix figé » de `SectionSelectionAttitudes` (« l'entretien 1 est signé » → « l'entretien tripartite est signé »). Assertion anti-régression ajoutée dans `pilotage-alertes.spec.ts` (le libellé n'était couvert par aucun test)
...
### Vague 10 juillet 2026 — documents administratifs nominatifs (demande direction)

#### La « partie 1 » du livret papier : dépôt, flag « réservé », attestation signée obligatoire (10 juillet 2026)

Demande de la direction (cadrage et arbitrages pilote dans
[`chantier-demandes-direction-2026-07.md`](chantier-demandes-direction-2026-07.md)) :
les documents administratifs du livret papier (engagements, convention,
règlement…) rejoignent la maquette — **nominatifs, par apprenti·e**, avec
**attestation de prise de connaissance obligatoire** par signature manuscrite.

- **Type `DocumentAdministratif`** : nominatif (`apprentiId`), fichier en
  data-URL (maquette — étape 2 : référence Nuage), flag `reserveApprenti`,
  attestation `SignaturePartie` portée par le document
- **Lib pure `documents-administratifs`** (11 tests TDD) :
  `peutConsulterDocument` (réservé → apprenti·e + coordo + admin ; sinon tous),
  `documentsApprentiVisibles`, `peutSupprimerDocument` (**document attesté
  insupprimable** — esprit R21, on remplace par un nouveau dépôt),
  `documentsNonAttestes`, `validerDepotDocument` (PDF/JPEG/PNG, ≤ 2 Mo)
- **Droits** : `documents.gerer` → **coordo + admin** (dépôt / retrait / flag) ;
  `documents.attester` → **apprenti·e seul·e** (acte personnel — doctrine
  coordo/admin sans droit pédagogique intacte). Matrice 55 → **57 ressources**
- **Store `useDocumentsStore`** (`livret-documents` v1 — 12ᵉ store) : dépôt,
  attestation (no-op si déjà signée), suppression gardée, reset ; fixtures
  **Léa ×2** (« Partie 1 » attestée ; « Convention » **réservée** non attestée)
...
- **UI** : page **`/livret/documents`** (sidebar tous rôles, **juste après le
  ...
  (nouvelles props `mentionRetrait` / `libelleBouton` — la mention R10 ne
  s'applique pas ici), suppression 2 clics (désactivée si attesté)
- **Suivi de l'obligation** : alertes « Document « X » : signature de
  l'apprenti·e attendue » (formateur hors documents réservés, coordo/admin
  tout — type `document-a-attester`), **bandeau cliquable** sur le tableau de
  bord apprenti·e (`TableauBordApprenti`)
- **PDF** : page « Documents administratifs » **en tête du livret complet**
  (miroir du livret papier) — titre (ou « Document réservé à l'apprenti·e »,
  sans titre : le PDF circule), déposé le/par, attestation horodatée + image
  de la signature ; branchée dans `useDonneesLivretPdf` et la page Synthèse
- **Étape 2 (tracé dans TODO-etape-2.md)** : binaires sur **Nuage**
  (`apps.education.fr`) via WebDAV backend — cf. `STACK_GRETA_LYON.md` §3.4 et
  §7.5-7.7 (RGPD documents nominatifs)
- Tests : **715 unitaires** (52 fichiers — +11 `documents-administratifs`,
  +2 `alertes` (18), +1 `droits` (54)) ; **213 E2E / 30 specs** (nouveau
  `documents-administratifs.spec.ts`, 5 scénarios : visibilité par rôle,
  signature tactile persistée, dépôt + verrou de suppression, alertes
  filtrées, bandeau apprenti·e)

### Vague 13 juillet 2026 — documents administratifs v2 (réunion DG, demande 3)

#### Typologie de 4 documents obligatoires + attestation sans signature (13 juillet 2026)

Première demande de la série issue de la réunion pilote × directeur général du
2026-07-13 (arbitrages dans
[`chantier-demandes-direction-2026-07.md`](chantier-demandes-direction-2026-07.md) —
demande 3). Le module documents (10 juillet) évolue sur trois axes : typologie
fermée, attestation allégée mais conditionnée à la lecture, anomalie de dépôt.

- **Typologie** (`TypeDocumentAdministratif`) : 4 types **OBLIGATOIRES** —
  Contrat pédagogique, Information relative à la protection des données,
  Droit à l'image, Accusé réception du règlement intérieur — + « **Autre
  document** ». La liste déroulante de la modale impose le type ; le **titre
  n'est saisi que pour « Autre »** (les 4 types tirent leur libellé de la
  typologie — `libelleDocument`), le **flag « réservé à l'apprenti·e » est
  refusé hors « Autre »** (les 4 obligatoires restent visibles de tous)
- **Attestation sans signature** (`AttestationLecture`) : plus de tracé
  manuscrit — bouton « J'atteste avoir pris connaissance » à **confirmation
  explicite** (2 clics, Esc / 30 s), **horodaté** (esprit R19), **sans retrait**
  (esprit R21 — document attesté toujours insupprimable). **« Lu et attesté »** :
  le bouton reste grisé tant que l'apprenti·e n'a pas **consulté** le document
  (première consultation tracée dans `consulteParApprentiLe` au clic
  « Consulter » — rôle apprenti·e uniquement ; verrou `peutAttesterDocument`)
- **Remplacement par type** : redéposer un type déjà déposé (même attesté)
  **remplace** l'ancien document — l'attestation repart de zéro, un
  avertissement le rappelle dans la modale (`depot-doc-remplacement`).
  ⚠ Maquette : l'ancien binaire est supprimé (budget localStorage ~5 Mo) —
  l'archivage des versions se fera sur Nuage en étape 2
- **Anomalie « document manquant »** (type `document-manquant`, icône dédiée) :
  une alerte **coordo/admin** par type obligatoire non déposé et par
  apprenti·e — « Document « X » : dépôt à effectuer (obligatoire) ». Le
  formateur n'est pas concerné (il ne dépose pas). L'alerte « attestation de
  l'apprenti·e attendue » (ex-« signature ») est conservée
- **Bandeau d'état des 4 obligatoires** sur `/livret/documents`
  (`etat-documents-obligatoires`) : attesté (vert) / attestation attendue
  (ambre) / non déposé (rouge) — identique pour tous les rôles (les 4 types ne
  sont jamais réservés)
- **PDF** : la page « Documents administratifs » est **toujours présente** dans
  le livret complet — état des 4 obligatoires (y compris « Document non déposé
  (obligatoire) ») puis « Documents complémentaires » (réservés toujours sans
  titre)
- **Store `livret-documents` v2** (reset fixtures) : `deposerDocument` typé
  avec remplacement, `marquerConsultationApprenti`, `attesterDocument` sans
  tracé (gardé par la consultation). Fixtures : **6 dossiers complets attestés**
  (Théo, Sofia, Camille, Minh, Aya, Luca — le centre d'alertes de démo reste
  lisible) + cas de démo **Léa** (contrat + règlement attestés, protection des
  données à attester, **droit à l'image MANQUANT**, convention « Autre »
  réservée) et **Yanis** (contrat à attester, visible de Marc TISSIER)
- Tests : **730 unitaires** (+12 `documents-administratifs` (23), +3 `alertes`
  (21)) ; **214 E2E / 30 specs** (`documents-administratifs.spec.ts` réécrit,
  6 scénarios)

### Vague 13 juillet 2026 — apprentis mineurs : responsables légaux (réunion DG, demande 5)

#### 6ᵉ rôle « Responsable légal » + attestation en lieu et place du mineur (13 juillet 2026)

Troisième demande de la série DG du 2026-07-13 (arbitrages dans
[`chantier-demandes-direction-2026-07.md`](chantier-demandes-direction-2026-07.md) —
demande 5) : les apprenti·e·s **mineur·e·s** à l'inscription ont 1 à 2
**responsables légaux**, qui attestent leurs documents administratifs et
consultent le reste du livret en lecture seule.

- **Minorité recalculée en continu** (lib pure `minorite` — `estMineur`,
  18 ans au jour près, années bissextiles gérées) : à la majorité,
  l'apprenti·e récupère automatiquement l'attestation (arbitrage 1)
- **Modèle** : `ResponsableLegal` (6ᵉ type d'utilisateur — prénom, nom,
  email + téléphone / lien de parenté optionnels), relation portée par
  `Apprenti.responsableLegalIds` (1-2). **Rattachement fratrie** : un email
  déjà connu comme responsable désigne la même personne
  (`enregistrerResponsablesApprenti` — création OU rattachement). Bump
  `livret-utilisateurs` v7 (**Minh NGUYEN devient MINEUR**, né en 2009 ;
  responsables de démo **Thi** (mère, défaut du sélecteur) et **Duc** (père))
- **Inscription manuelle** (`ModaleApprenti`) : section « Responsables
  légaux » conditionnée à la minorité de la date saisie (recalcul en direct),
  2 blocs (le 1ᵉʳ obligatoire), validation partagée
  (`validerResponsablesLegaux` : 1 minimum si mineur·e, emails valides,
  ≠ apprenti·e, distincts entre eux, uniques sauf responsable connu)
- **Import Excel** : 10 colonnes « resp. légal 1/2 » (prénom, nom, email,
  téléphone, lien) ajoutées au modèle téléchargeable — **optionnelles à
  l'en-tête** (un fichier sans elles reste importable pour des majeur·e·s),
  1 responsable exigé par ligne MINEURE, création / rattachement à
  l'insertion. Exemple mineur ajouté au modèle
- **6ᵉ rôle « Responsable légal »** (couleur **cyan** `#0e7490` — 6ᵉ entrée
  de toute la chaîne graphique : tailwind, `role-actif-*`, `couleurs-role`,
  RoleSwitcher, AppShell) : sélecteur de responsable actif sur le tableau de
  bord (pattern formateurs — lien de parenté affiché), périmètre
  `apprentis-accessibles` limité à ses enfants, **aucun menu
  d'administration** (liens gardés par ressource)
- **Droits** : `documents.attester` s'ouvre à `responsable` — c'est
  `attestataireDocuments(apprenti)` (minorité au jour) qui tranche qui est
  actif ; `entretien.signature-representant-legal` s'ouvre à `responsable`
  (il signe LUI-MÊME le slot optionnel, identité remplie depuis son compte —
  le formateur garde le fallback, arbitrage 7). **Test transverse** : balayage
  exhaustif de la matrice — le responsable est en lecture seule partout sauf
  ces 2 ressources (`TOUTES_RESSOURCES` exporté)
- **Documents** : l'attestation trace son **auteur·rice** (`attestePar*` —
  bump `livret-documents` v4, wording UI/PDF « attestée par X, responsable
  légal, le… », libellé historique conservé), « lu et attesté » déverrouillé
  par la consultation du RESPONSABLE pour un·e mineur·e, cercle des documents
  « réservés » élargi au responsable (arbitrage 5), bandeau apprenti·e masqué
  pour un·e mineur·e (mention dédiée sur la page), alertes « attestation du
  responsable légal attendue » (encadrement) et « votre attestation est
  attendue » (centre d'alertes du responsable). Fixture : la protection des
  données de Minh attend l'attestation de ses responsables
- **Administration** : les responsables apparaissent dans la liste des
  utilisateurs (lecture — « Géré via l'apprenti·e », filtre par rôle dédié) ;
  leur gestion passe par la fiche de l'apprenti·e mineur·e
- **⚠ RGPD** : la réintroduction des mineurs (retirés au cadrage du
  2026-05-26) impose de **réexaminer le dossier étape 2 avec le DPO** — AIPD
  probablement requise (personnes vulnérables, critère CNIL). Consigné au
  chantier ; création des comptes responsables tracée dans `TODO-etape-2.md`
- Tests : **767 unitaires** (54 fichiers — +`minorite` (4),
  +`responsables-legaux` (9), import (28), droits (55), documents (37),
  apprentis-accessibles (34)) ; **220 E2E / 31 specs** (nouveau
  `responsables-legaux.spec.ts`, 4 scénarios : inscription manuelle d'une
  mineure, périmètre / lecture seule du rôle, attestation en lieu et place +
  apprenti mineur en lecture, import Excel mineure refusée puis créée)

### Vague 13 juillet 2026 — documents au niveau formation (réunion DG, demande 4)

#### Dépôt en masse pour toute une promo (13 juillet 2026)

Deuxième demande de la série DG du 2026-07-13 (arbitrages dans
[`chantier-demandes-direction-2026-07.md`](chantier-demandes-direction-2026-07.md) —
demande 4) : les documents génériques (règlement intérieur…) se déposent **une
fois pour toute la formation** au lieu d'apprenti·e par apprenti·e.

- **Nouvelle entité `DocumentFormation`** : rattachée à `formationId`, stockée
  UNE fois (pas de copie — le localStorage de la maquette est compté), avec
  `consultations` et `attestations` **indexées par apprenti·e** (attestation
  individuelle après lecture, comme la demande 3). Couvre les apprenti·e·s
  actuels **et futurs** de la promo (arbitrage 1)
- **Types en masse** : `TYPES_DOCUMENT_FORMATION` = tous **sauf
  `contrat-pedagogique`** (nominatif par nature — arbitrage 2) ; jamais de
  flag « réservé » (document générique). Validation dédiée
  (`validerDepotDocumentFormation`)
- **Fusion `documentsEffectifsApprenti`** (lib pure) : nominatifs visibles +
  documents de la formation **projetés** par apprenti·e
  (`projeterDocumentFormation` — `porteeFormation: true`). **Le nominatif
  prime** à type égal (arbitrage 3), les « autre » coexistent. Utilisée
  PARTOUT : page Documents, bandeau apprenti·e, centre d'alertes (attestations
  attendues par apprenti·e — id préfixé — ET anomalies « manquant » couvertes
  par un dépôt de masse), état des 4 obligatoires, PDF (`useDonneesLivretPdf`,
  Synthèse)
- **Point d'entrée unique** (arbitrage 6) : page **Formations** — bouton
  « Documents (N) » par carte (`documents.gerer`) → `ModaleDepotDocument`
  généralisée (prop `cible` : apprenti·e OU formation) qui **liste les
  documents de la promo** (libellé, compteur « attesté par x/y », suppression
  tant que personne n'a attesté — `peutSupprimerDocumentFormation`, esprit
  R21, arbitrage 5) et permet le dépôt (la modale reste ouverte pour des
  dépôts successifs)
- **Remplacement en masse** (arbitrage 4) : redéposer un type déjà déposé pour
  la formation remplace le document — **toutes les attestations de la promo
  repartent de zéro** (avertissement dédié dans la modale)
- **UI apprenti·e** : badge « **Document de la formation** » sur la carte,
  suppression remplacée par la mention « Géré via Formations » (coordo/admin),
  attestation et consultation routées vers les mutations de formation
- **Store `livret-documents` v3** (reset fixtures) : sous-état
  `documentsFormation` + `deposerDocumentFormation` (remplacement),
  `marquerConsultationFormationApprenti`, `attesterDocumentFormation`,
  `supprimerDocumentFormation`. Fixtures : le **règlement intérieur devient un
  document de formation** (CAP + BTS) — attesté par tou·te·s **sauf Sofia**
  (fait vivre l'alerte « attestation attendue » d'un document de promo) ; les
  dossiers nominatifs passent à 3 types
- Tests : **745 unitaires** (+12 `documents-administratifs` (35), +3 `alertes`
  (24)) ; **216 E2E / 30 specs** (2 scénarios de dépôt en masse : couverture
  de la promo + attestation de l'apprentie, remplacement collectif avec
  remise à zéro)

### Vague 10 juillet 2026 — mode de pédagogie sur le tableau de bord (demande pilote)

#### Badge « Pédagogie en Compétences / en Activités » par groupe de formation (10 juillet 2026)

Sur le tableau de bord (vue groupée par formation — formateur, coordo, admin),
l'en-tête de chaque section affiche le **mode de pédagogie** de la formation, à
côté de l'intitulé et du compteur d'apprenti·e·s :

- Composant local `BadgePedagogie` (`TableauDeBord.tsx`) : « **Pédagogie en
  Compétences** » (gris) ou « **Pédagogie en Activités** » (violet) — dérivé de
  `modeEffectif(formation)` (chantier #4), même code couleur que le badge
  « Mode … » des pages Formations / Modèles d'activités ; testid
  `pedagogie-{formationId}`. Le groupe « Sans formation » n'a pas de badge
- L'en-tête du groupe passe en `flex-wrap` (le badge replie proprement sur
  mobile — audit débordement inchangé)
- Aucun changement de lib / store / droits ; compteurs de tests inchangés
  (assertions ajoutées au scénario « regroupées par formation » de
  `tableau-de-bord-6-apprentis.spec.ts` : CAP → Activités, BTS → Compétences)

### Vague 10 juillet 2026 — bascule en mode activités à balayage partiel (retour démo direction)

#### Le mode activités se débloque dès que chaque activité fait appel à au moins une compétence (10 juillet 2026)

Révision de l'arbitrage Q6 du chantier #4 après une nouvelle démonstration à la
direction : exiger le **balayage complet** du référentiel avant la bascule était
trop contraignant. Le mode activités se débloque désormais dès que **chaque
activité du modèle est mappée sur au moins une compétence évaluable** — peu
importe que toutes les compétences soient couvertes (les non couvertes
n'apparaissent pas dans la Synthèse, dont la grille était déjà restreinte aux
compétences couvertes par les activités retenues).

- **Lib `balayage-referentiel`** : nouvelle fonction **`activitesSansCompetenceEvaluable`**
  (activités au mapping vide ou pointant uniquement des ids disparus / exclus) —
  LA condition de bascule ; `calculerBalayage` (jauge X/Y) conservée à titre
  informatif ; `estCouverteParModele` supprimée avec son unique consommatrice
- **Lib `mode-evaluation`** : `peutBasculerMode` refuse uniquement si ≥ 1 activité
  sans compétence (raison listant les activités en défaut) ; **`peutReactiverCompetence`
  supprimée** (réactiver une exclue ne fait qu'ajouter une compétence évaluable —
  plus rien à protéger) ; **nouvelle garde `peutExclureCompetence`** : l'exclusion
  d'une compétence est bloquée si une formation rattachée est en mode activités et
  qu'une activité ne serait plus mappée sur rien (cas impossible sous l'ancien
  invariant, ouvert par le nouveau). Réimport / changement de référentiel toujours
  bloqués en mode activités (mappings orphelins)
- **Stores** : `useReferentielsStore.basculerExclusionCompetence` inverse sa garde
  (réactivation libre, exclusion contrôlée) ; `useActivitesStore.setMappingActivite`
  ne refuse plus que la retouche laissant une activité à 0 compétence (découvrir
  une compétence du référentiel devient libre, même en mode activités)
- **UI (`GestionActivites`)** : bouton « Passer en mode activités » conditionné aux
  activités toutes mappées ; encart ambre « ⚠ N activité(s) sans compétence mappée :
  … » listant les activités en défaut ; jauge de balayage **informative** (« X/Y
  compétences couvertes (partiel, non bloquant) », état vert dès que la condition
  de bascule est remplie) ; liste « Non couvertes » suffixée « (ces compétences
  n'apparaîtront pas dans la Synthèse) » ; textes d'intro alignés
- **Fixtures inchangées** (CAP Cuisine : 6 activités toutes mappées, balayage 10/10
  toujours valide) — **aucun bump**
- Tests : **701 unitaires** (51 fichiers — `balayage-referentiel` 11 → 13,
  `mode-evaluation` 16 → 17) ; **208 E2E** (scénario `admin-activites` réécrit :
  import BTS → bascule bloquée à 2 activités non mappées → 1 compétence sur chaque
  activité → **bascule débloquée à 1/11 de couverture**)

### Vague 8 juillet 2026 — points d'alerte de l'entretien au coordo / admin (demande pilote)

#### Les points d'alerte de l'entretien remontent dans « À traiter » et se cochent « traité » (8 juillet 2026)

Les **points d'alerte** de la trame de l'entretien tripartite (réponses oui/non
signalant une difficulté selon `alerteSi` — récapitulés en fin d'entretien sous
« Points d'alerte (X) : action à mener par le GRETA CFA ») ne servaient qu'à
l'affichage (entretien + PDF), sans suivi. Ils **remontent** désormais dans le
centre d'alertes « À traiter » du **coordo / admin**, qui peut les **cocher
« traité »**. **L'entretien tripartite reste strictement inchangé** : les points
sont dérivés de ses réponses (`pointsAlerteTrame`) et leur traitement est suivi à
part.

- **Lib pure `points-alerte`** (7 tests TDD) : `pointsAlerteNonTraites(livret)` —
  remonte les points d'alerte d'un entretien **signé par les 3 parties** (R9,
  réponses figées — décision pilote) moins ceux déjà traités ; `[]` si l'entretien
  est absent, non signé ou sans difficulté. `estPointAlerteTraite(livret, id)`
- **Modèle** : `Livret.pointsAlerteTraites?: string[]` (ids des questions de la
  trame traités) — **hors de `Livret.entretien`**, champ **optionnel** → **aucun
  bump** (les livrets sans le champ = aucun traité)
- **Alertes** (`lib/alertes`) : nouveau type `point-alerte-entretien`, généré pour
  coordo / admin (une entrée par point non traité, `questionId` + `livretId`
  portés par l'alerte), classé juste après les R7 ; message = libellé du point,
  lien vers l'entretien
- **Droits** : nouvelle ressource **`point-alerte.traiter`** → **`['coordo',
  'admin']`** — acte de **gestion** (n'écrit rien dans l'entretien ni les
  évaluations), conforme à la doctrine « coordo/admin sans droit pédagogique ».
  Matrice 54 → **55 ressources**
- **Store** : `basculerPointAlerteTraite(livretId, questionId)` (toggle dans
  `pointsAlerteTraites` ; cocher retire le point de « À traiter », décocher l'y
  ramène)
- **UI** (`CentreAlertes`) : rendu dédié pour ce type — icône drapeau, libellé
  cliquable (ouvre l'entretien pour le contexte) + **bouton « Traité » à
  confirmation à 2 clics** (« Confirmez-vous avoir réglé ce problème ? » [✓] [✗],
  pattern de `BoutonSupprimer` — Esc / 10 s annulent) ; câblée dans
  `TableauDeBord` (gated par `point-alerte.traiter`). Les autres rôles ne voient
  pas ces alertes ; l'apprenti·e / maître gardent leur vue
- **Démo** : Léa (entretien signé 3/3, 2 points d'alerte — absences + logement)
  fait remonter 2 lignes cochables chez sa coordinatrice Martine
- Tests : **698 unitaires** (51 fichiers — +7 `points-alerte`, +4 `alertes`, +1
  `droits`) ; **208 E2E** (+1 scénario `pilotage-alertes` : remontée + cochage →
  disparition)

### Vague 8 juillet 2026 — activité libre détaillable (retour pilote)

#### Saisie de l'intitulé et des détails d'une activité libre en période entreprise (8 juillet 2026)

Sur une fiche de période en entreprise, l'ajout d'une **activité libre** (option
« + Activité libre (hors modèle) » en mode activités, « + Activité libre (hors
référentiel) » en mode compétences) créait une ligne au libellé **figé** « Activité
libre », sans moyen de la nommer ni de la décrire. Le tuteur ne pouvait donc pas
donner le **contexte** de l'activité réellement réalisée. La colonne « Activité »
présente désormais, pour ces lignes ad hoc, une **zone de texte éditable** (intitulé
+ contexte / détails).

- **Lib pure `lignes-suivi`** (4 tests TDD) : `estLigneLibre(ligne)` — une ligne de
  suivi entreprise est « libre » (ad hoc) quand elle ne porte NI compétence NI
  activité ; son intitulé n'est alors résolu depuis aucune source et se saisit dans
  `libelleLibre`
- **Store** : `setEvaluationLigne` étendu d'un variant `{ type: 'libelleLibre' }`
  (auto-horodatage `modifieLe` via `muterFiche`) — pas de nouvelle mutation
- **UI (`TableauTriColonnes`)** : nouveau sous-composant `ChampActiviteLibre`
  (`<textarea>` 3 lignes, placeholder « Nommez et décrivez l'activité réalisée
  (contexte, détails)… »), rendu à la place du libellé figé dès qu'une ligne est
  libre ET que le rôle peut éditer la fiche — desktop (colonne « Activité ») et
  mobile (en-tête de carte). Verrou d'édition = `peutAjouterLigne`
  (`fiche.ajouter-competence` : **formateur + maître / tuteur**, tant que la fiche
  reste éditable pour le rôle) — apprenti·e / coordo / admin en lecture seule
- **PDF** : inchangé (la colonne « Activité » affiche déjà `libelleLibre` avec
  repli « Activité hors modèle / hors référentiel » si vide)
- **Aucun bump de schéma** (le champ `libelleLibre` préexiste ; les 2 modes le
  partageaient déjà) ; la fixture de démo (Luca P3 « Inventaire de fin de mois en
  économat ») devient éditable telle quelle
- Tests : **686 unitaires** (50 fichiers — +4 `lignes-suivi`) ; **207 E2E / 29 specs**
  (+1 scénario dans `entretien-selection-competences.spec.ts` : ajout d'une activité
  libre → zone de texte → saisie → persistance après rechargement)

### Vague 7 juillet 2026 — auto-réparation des livrets manquants (bug pilote)

#### Un apprenti créé par l'utilisateur perdait son livret à chaque bump (7 juillet 2026)

La politique de migration (bump de `VERSION_SCHEMA` = reset aux fixtures) est **par
store** : un bump de `livret-donnees` effaçait les livrets des apprenti·e·s créé·e·s
par l'utilisateur (conservé·e·s, eux/elles, dans `livret-utilisateurs`, non bumpé).
L'apprenti·e restait visible au tableau de bord mais l'ouverture de son livret
retombait sur « Aucun·e apprenti·e sélectionné·e » (cas signalé : Marcel RAMOS chez
Marc TISSIER).

- **Lib** : `reparation-livrets.ts` — `apprentisSansLivret(apprentis, livrets)` (4 tests TDD)
- **Store** : `useLivretStore.reparerLivretsManquants()` — recrée un livret vierge par
  orphelin·e (planning entreprise + centre hérités de la formation, sélection de
  compétences réalignée « tout coché » sur le référentiel EFFECTIF, sélection
  d'activités réalignée sur le modèle si la formation est en mode activités)
- **AppShell** : appel idempotent au montage de l'application
- **Correctif connexe** : `ajouterApprenti` réaligne désormais aussi la sélection de
  compétences sur le référentiel de la formation (avant : ids CAP en dur via
  `creerLivretVierge`, faux pour une création BTS)
- **E2E** : nouveau spec `reparation-livrets.spec.ts` (suppression du livret de Léa
  dans localStorage → reload → le livret s'ouvre, planning hérité, entretien en
...
## 6. Tests (767 unitaires + 220 E2E)

### Tests unitaires Vitest (54 fichiers de test)

| Fichier                                        | Tests  | Périmètre                                                                                                                                                           |
| ---------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `lib/droits.test.ts`                           | 55     | Matrice des ressources × 6 rôles (organisation-suivi partagée, `entretien.gestion`, **co-saisie des champs du maître par le formateur (1ᵉʳ juillet)**, **`fiche.attitudes` maître seul (6 juillet)**, **`point-alerte.traiter` coordo / admin (8 juillet)**, **`documents.gerer` / `documents.attester` (10 juillet)**, **responsable légal en lecture seule partout sauf attestation + slot représentant légal — balayage exhaustif (13 juillet)**) |
...
| `lib/apprentis-accessibles.test.ts`            | 34     | Filtre par rôle (R3) + tri + recherche + **référent direct hors promoIds** + **groupement par formation** + **périmètre du 2ᵉ formateur (3 juillet 2026)** + **périmètre du responsable légal (13 juillet 2026)** |
...
| **`lib/alertes.test.ts`**                       | **24** | **Centre d'alertes par rôle : signatures attendues, verrouillages, initialisations, R7, affectations incomplètes (3 juillet 2026) + points d'alerte de l'entretien (8 juillet) + documents non attestés (10 juillet — formateur hors réservés) + documents obligatoires MANQUANTS coordo/admin et documents de FORMATION par apprenti·e (13 juillet)** |
| **`lib/points-alerte.test.ts`**                 | **7**  | **Points d'alerte de l'entretien à traiter par la coordination : remontée conditionnée à la signature 3/3, exclusion des points marqués « traités » (8 juillet 2026)** |
| **`lib/documents-administratifs.test.ts`**      | **37** | **Documents (10 juillet 2026 ; v2/v3/v4 13 juillet) : typologie (4 obligatoires + « autre », libellés), visibilité du flag « réservé » (responsable inclus), verrous de suppression et d'attestation (« lu et attesté »), types manquants + état des obligatoires, validation de dépôt (titre pour « autre » seul, réservé refusé hors « autre », type / taille), documents de FORMATION (fusion `documentsEffectifsApprenti` — projection, nominatif prime, types en masse sans contrat, verrou de suppression à la 1ʳᵉ attestation) + `attestataireDocuments` (mineur → responsable légal, demande 5)** |
...
| **`lib/import-utilisateurs.test.ts`**          | **28** | **Chantier #5 : parsing 3 modèles + validation + politique tout-ou-rien + responsables légaux des mineur·e·s (colonnes optionnelles, 1 exigé si mineur·e, rattachement fratrie — 13 juillet 2026)** |
...
| **`lib/balayage-referentiel.test.ts`**         | **13** | **Jauge de balayage du mapping (couvertes / manquantes / orphelines, exclusions — chantier #4) + `activitesSansCompetenceEvaluable` (condition de bascule, 10 juillet 2026)** |
...
| **`lib/mode-evaluation.test.ts`**              | **17** | **Verrous du mode d'évaluation (première saisie signée, chaque activité mappée sur ≥ 1 compétence — révision du 10 juillet 2026, gardes référentiel dont `peutExclureCompetence` — chantier #4)** |
...
| **`lib/lignes-suivi.test.ts`**                 | **4**  | **`estLigneLibre` : une ligne de suivi entreprise sans compétence ni activité est « libre » (intitulé + détails saisis dans `libelleLibre`, 8 juillet 2026)** |
...
├── e2e/                            # tests Playwright (31 specs — dont responsables-legaux, 13 juillet)
...
└── src/
    ...
    ├── lib/                        # 57 modules + 54 fichiers tests
    │   ├── droits.ts               # matrice §6 (57 ressources × 6 rôles)
    ...
    │   ├── alertes.ts              # centre d'alertes par rôle (3 juillet) + points d'alerte entretien (8 juillet) + documents (attestations, manquants, responsable — 10-13 juillet)
    │   ├── points-alerte.ts        # points d'alerte de l'entretien à traiter par la coordination (8 juillet)
    │   ├── reparation-livrets.ts   # auto-réparation des livrets manquants après bump (7 juillet)
    │   ├── lignes-suivi.ts         # `estLigneLibre` — ligne de suivi ad hoc (activité libre détaillable, 8 juillet)
    │   ├── documents-administratifs.ts # documents : typologie, visibilité, verrous, fusion nominatif/formation, attestataire mineur (10-13 juillet)
    │   ├── minorite.ts             # estMineur — minorité recalculée au jour (13 juillet, demande 5)
    │   ├── responsables-legaux.ts  # validation des responsables légaux, partagée modale/import (13 juillet)
    ...
    │   ├── useUtilisateursStore.ts # v7 — +responsables légaux, Minh MINEUR (13 juillet, demande 5)
    ...
    │   └── useDocumentsStore.ts    # v4 — documents administratifs (10 juillet ; typologie + attestation après lecture + documents de formation + auteur d'attestation 13 juillet)
    ├── fixtures/                   # 8 livrets démo (6 CAP + 2 BTS MHR) + utilisateurs (dont Minh MINEUR + responsables Thi/Duc — 13 juillet) + formations + 2 référentiels (dont BTS 3 niveaux) + documents administratifs (pdf-demo.ts, documents-demo.ts — typologie, règlement porté par la formation, protection des données de Minh à attester)
    ├── components/
    │   ├── admin/
    │   │   ├── ModaleApprenti.tsx
    ...
    │   │   └── ModaleDepotDocument.tsx      # dépôt d'un document administratif — portée apprenti·e OU formation (10-13 juillet)
    │   ├── layout/
    ...
    │   ├── DocumentsAdministratifs.tsx      # documents + attestation après lecture, attestataire mineur/responsable (10-13 juillet)
    │   ├── PronoteWeb.tsx
    │   ├── AccesMobile.tsx                  # QR code d'accès (encadrement — 3 juillet)
    │   └── admin/
    │       ├── GestionUtilisateurs.tsx      # bordures gauches colorées
    │       ├── GestionFormations.tsx        # +boutons « Planning » et « Documents » (dépôt en masse, 13 juillet)
...
npm run e2e            # 220 tests E2E Playwright (build + preview + tests)
...
L'étape 1 du CDC v1.3 est **livrée et fonctionnelle**, enrichie par les vagues post-livraison successives (dernière : réunion DG du 13 juillet 2026). Pour passer en étape 2, le pilote a identifié 3 chantiers majeurs structurellement liés :
...
Pour les comptes **non couverts par le SSO** (apprenti·e·s + maîtres d'apprentissage + **responsables légaux des mineur·e·s** (13 juillet 2026 — demande 5, données déjà saisies en maquette) = personnes hors GRETA), il faut un mécanisme de création avec validation par email :
...
_Étape 1 livrée + vagues post-livraison successives (CDC v1.5, chantiers métier mai-juillet 2026, demandes direction — dernière vague : réunion DG du 13 juillet 2026 : documents typés obligatoires, dépôt en masse par formation, responsables légaux des mineurs / 6ᵉ rôle). **Fonctionnel GELÉ le 2026-07-13 — phase de déploiement o2switch lancée** (prérequis administratifs : [`chantier-deploiement-o2switch.md`](chantier-deploiement-o2switch.md) ; demande 2 reportée post-déploiement). Prochaine étape : SSO Entra + gestion comptes/mots de passe (+ réexamen RGPD mineurs avec le DPO) — cf. §12._
