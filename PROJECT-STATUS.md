...
| État des tests (611 unit + 196 E2E)                             | §6                                                                                                        |
...
| **Tests unitaires**   | **611 / 611 ✓** (Vitest, 40 fichiers de test)                                                    |
| **Tests E2E**         | **196 / 196 ✓** (Playwright — Chromium desktop + mobile Pixel 5, 27 specs)                       |
...
- **State** : Zustand 5 + middleware `persist` — **10 stores** persistés en localStorage :
  - `livret-donnees` (schema v24) — livrets, fiches **entreprise** (compétences + **évaluations des attitudes par période** — juillet 2026) **+ centre de formation** (`fichesSuiviCentre`, **observations de fin de période seules**), **l'entretien tripartite unique et obligatoire** (`Livret.entretien` — trame officielle GRETA), synthèse des compétences (colonne entreprise seule), sélection des compétences abordées en entreprise, **forçage d'affichage des périodes** (`affichagePeriodesForce`)
...
└── src/
    ...
    │   ├── useUserStore.ts
    │   ├── useLivretStore.ts       # v24 — fiches centre simplifiées + attitudes par période
