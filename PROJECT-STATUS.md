...
L'**étape 1 du CDC v1.3 est livrée et déployée**, enrichie par 7 vagues post-livraison (CDC v1.5 + chantiers métier mai 2026 + retours coordonnateurs juin 2026 + réunion direction 1ᵉʳ juillet 2026). La maquette est fonctionnelle, accessible sur URL publique avec Basic Auth, et tous les flux pédagogiques sont testés en bout-en-bout : **611 tests unitaires + 194 tests E2E passent**, bundle JS gzippé sous 170 KB. Aucune authentification réelle ni backend persistant pour l'instant — c'est précisément l'objet de l'étape 2.
...
| État des tests (611 unit + 194 E2E)                             | §6                                                                                                        |
...
| **Tests unitaires**   | **582 / 582 ✓** (Vitest, 36 fichiers de test)                                                    |
| **Tests E2E**         | **180 / 180 ✓** (Playwright — Chromium desktop + mobile Pixel 5, 25 specs)                       |
...
#### Retrait du « Suivi GRETA CFA » des fiches entreprise (1ᵉʳ juillet 2026 — réunion direction)

- La zone « Suivi de la formation au GRETA CFA » faisait doublon sur les périodes en entreprise depuis l'arrivée des périodes en centre, qui portent ce suivi. Retirée des fiches entreprise (page + PDF), conservée sur les fiches en centre
- La règle de signature R20 du formateur suit (avant son remplacement par la vague « 2 signatures » ci-dessous)
...
## 6. Tests (611 unitaires + 194 E2E)

### Tests unitaires Vitest (39 fichiers de test)

| Fichier                                        | Tests  | Périmètre                                                                                                                                                           |
| ---------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `lib/droits.test.ts`                           | 50     | Matrice 50 ressources × 5 rôles (organisation-suivi partagée, `entretien.gestion`, **co-saisie des champs du maître par le formateur (1ᵉʳ juillet)**, `admin.entreprises.gerer`) |
...
| `lib/validation-signature.test.ts`             | 19     | R18/R20 par lieu — **le formateur ne signe plus en entreprise (1ᵉʳ juillet 2026)**                                                                                  |
...
| `lib/attitudes.test.ts`                        | 10     | Catalogue par défaut (12 attitudes — a1..a4 retirées le 18 juin) + `attitudeEstUtilisee` + `auMoinsUneAttitudeEvaluee`                                               |
...
| `lib/apprentis-accessibles.test.ts`            | 32     | Filtre par rôle (R3) + tri + recherche + **référent direct hors promoIds** + **groupement par formation (1ᵉʳ juillet 2026)**                                        |
...
```bash
npm test               # 611 tests Vitest
npm run e2e            # 194 tests E2E Playwright (build + preview + tests)
