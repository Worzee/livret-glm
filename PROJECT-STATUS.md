...
L'**étape 1 du CDC v1.3 est livrée et déployée**, enrichie par 8 vagues post-livraison (CDC v1.5 + chantiers métier mai 2026 + retours coordonnateurs juin 2026 + réunion direction 1ᵉʳ juillet 2026). La maquette est fonctionnelle, accessible sur URL publique avec Basic Auth, et tous les flux pédagogiques sont testés en bout-en-bout : **715 tests unitaires + 213 tests E2E passent**, bundle JS gzippé ~185 KB. Aucune authentification réelle ni backend persistant pour l'instant — c'est précisément l'objet de l'étape 2.
...
| **Tests unitaires**   | **715 / 715 ✓** (Vitest, 52 fichiers de test)                                                    |
| **Tests E2E**         | **213 / 213 ✓** (Playwright — Chromium desktop + mobile Pixel 5, 30 specs)                       |
...
- **State** : Zustand 5 + middleware `persist` — **12 stores** persistés en localStorage :
  ...
  - `livret-documents` (schema v1) — **documents administratifs nominatifs** (10 juillet 2026, demande direction) : fichiers en data-URL (≤ 2 Mo — étape 2 : Nuage/WebDAV), flag « réservé à l'apprenti·e », attestation signée (tactile) par document
...
| **`lib/alertes.test.ts`**                       | **18** | **Centre d'alertes par rôle : signatures attendues, verrouillages, initialisations, R7, affectations incomplètes (3 juillet 2026) + points d'alerte de l'entretien (8 juillet) + documents non attestés (10 juillet — formateur hors réservés)** |
...
| **`lib/documents-administratifs.test.ts`**      | **11** | **Documents nominatifs (10 juillet 2026) : visibilité du flag « réservé », filtre par apprenti·e, verrou de suppression d'un document attesté, validation de dépôt (type / taille)** |
