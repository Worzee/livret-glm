...
L'**étape 1 du CDC v1.3 est livrée et déployée**, enrichie par 8 vagues post-livraison (CDC v1.5 + chantiers métier mai 2026 + retours coordonnateurs juin 2026 + réunion direction 1ᵉʳ juillet 2026). La maquette est fonctionnelle, accessible sur URL publique avec Basic Auth, et tous les flux pédagogiques sont testés en bout-en-bout : **745 tests unitaires + 216 tests E2E passent**, bundle JS gzippé ~185 KB. Aucune authentification réelle ni backend persistant pour l'instant — c'est précisément l'objet de l'étape 2.
...
| État des tests (745 unit + 216 E2E)                             | §6                                                                                                        |
...
| **Tests unitaires**   | **745 / 745 ✓** (Vitest, 52 fichiers de test)                                                    |
| **Tests E2E**         | **216 / 216 ✓** (Playwright — Chromium desktop + mobile Pixel 5, 30 specs)                       |
...
## 6. Tests (745 unitaires + 216 E2E)

### Tests unitaires Vitest (52 fichiers de test)

| Fichier                                        | Tests  | Périmètre                                                                                                                                                           |
| ---------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `lib/droits.test.ts`                           | 54     | Matrice des ressources × 5 rôles (organisation-suivi partagée, `entretien.gestion`, **co-saisie des champs du maître par le formateur (1ᵉʳ juillet)**, **`fiche.attitudes` maître seul (6 juillet)**, **`point-alerte.traiter` coordo / admin (8 juillet)**, **`documents.gerer` / `documents.attester` (10 juillet)**) |
...
| **`lib/documents-administratifs.test.ts`**      | **35** | **Documents (10 juillet 2026 ; v2/v3 13 juillet) : typologie (4 obligatoires + « autre », libellés), visibilité du flag « réservé », verrous de suppression et d'attestation (« lu et attesté »), types manquants + état des obligatoires, validation de dépôt (titre pour « autre » seul, réservé refusé hors « autre », type / taille) + documents de FORMATION (fusion `documentsEffectifsApprenti` — projection, nominatif prime, types en masse sans contrat, verrou de suppression à la 1ʳᵉ attestation)** |
...
└── src/
    ...
    │   └── useDocumentsStore.ts    # v3 — documents administratifs (10 juillet ; typologie + attestation après lecture + documents de formation 13 juillet)
