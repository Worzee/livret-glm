...
export function FicheSuiviPeriodeDetail({ lieu = 'entreprise' }: FicheSuiviPeriodeDetailProps) {
  ...
  // directe. Le coordo / admin (supervision) y accèdent toujours ; et si la
  // coordination a forcé l'affichage, tous les rôles y accèdent (18 juin 2026).
  const voirTout =
    peutContournerSequencement(roleActif) || (livret.affichagePeriodesForce?.[lieu] ?? false);
  if (!estPeriodeVisible(toutesFiches, fiche.id, lieu, voirTout)) {
    return (
