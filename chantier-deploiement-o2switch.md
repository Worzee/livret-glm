...
| 2026-08-28 | **Refonte documentaire — impact de déploiement complet (6 lots).** ⚠ **DEUX SQL AVANT LE BUNDLE**, dans cet ordre : `2026-08-28-documents-communs.sql` (1 table — sans elle, **la création d'une formation échoue**, pas seulement la nouvelle page) puis `2026-08-28-attestation-responsable.sql` (4 colonnes nullables, **aucune reprise de données** : les attestations déjà signées par un responsable sont RELUES comme telles). ⚠⚠ **PIÈGE phpMyAdmin RENCONTRÉ AU MOMENT DE JOUER LE PREMIER SQL** : import lancé depuis la RACINE → « L'importation a réussi, 3 requêtes exécutées » en VERT **et** `#1046 - Aucune base n'a été sélectionnée` en ROUGE, **rien n'ayant été créé** (le vert compte les requêtes ENVOYÉES). C'est le symétrique exact du piège du 27 août, où le rouge portait sur la vérification alors que la table existait : **le bandeau vert ne prouve rien, dans les deux sens** — seul `SHOW TABLES LIKE '…';` fait foi. Correction : cliquer sur `tlxn8907_livret` dans le panneau de gauche AVANT l'onglet « Importer ». Consigné en tête des deux SQL, au tableau de dépannage du kit et au runbook §14. **Bundle** : 26,6 Mo, 1 396 fichiers, préflight attendu 20/20 (aucune page publique ajoutée). ⚠⚠ **AUTRE PIÈGE, COÛTEUX** : le premier bundle est sorti à **96 Mo** — trois archives (`livret-bundle.tar.gz` et deux `.tar.gz.old`) traînaient à la racine et le sur-tracing les a embarquées. Le motif d'exclusion du runbook §1 ne visait qu'un nom précis depuis juillet ; il est passé à `'./livret-bundle*'`, et surtout **`package-standalone.mjs` purge désormais toute archive du standalone** — un bundle ne contient jamais un autre bundle. **Mise en service après bascule** : déposer les 10 documents communs au catalogue, PUIS compléter les formations existantes une par une. ⚠ **À dire aux coordonnateurs AVANT** : déposer un contrat pédagogique REFERME le livret de l'apprenti·e jusqu'à son attestation — un dépôt en masse de rentrée limite toute la promotion aux documents administratifs. |
| 2026-08-28 | **Point médian proscrit — DÉPLOYÉ le 2026-08-29** (SQL importé puis bundle remplacé, rapporté par le pilote). Retour de la DIRECTION : la circulaire du ministère de l'Éducation nationale du 5 mai 2021 (BO n° 18) proscrit le point médian, interdiction que le Conseil d'État a jugée légale et justifiée pour l'enseignement. Toute la plateforme est réécrite au **masculin générique** (2 117 occurrences, 217 fichiers, 108 formes, plus 98 doublets abrégés par barre oblique). ⚠ **UN SQL** : `prisma/deploiement/2026-08-28-suppression-point-median.sql` — le catalogue des questions d'entretien annuel est seedé **puis la base fait foi**, si bien que **9 des 30 questions portent la graphie EN BASE** et qu'aucun bundle ne les corrige ; c'est le SEUL endroit de la base concerné. `REPLACE()` ciblé plutôt qu'écrasement des 30 libellés, pour préserver les retouches faites depuis la page admin ; rejouable ; équivalence « texte du SQL » ↔ « texte du code » vérifiée ligne à ligne avant écriture. **L'ordre SQL/bundle est libre** : aucune dépendance technique, rien ne casse si le SQL est joué après — les 9 questions gardent simplement l'ancienne graphie d'ici là. Aucune variable d'environnement, aucune table, **préflight inchangé à 20/20** (aucune page publique ajoutée). **Recette** : un écran par rôle, les 9 questions dans `/admin/questions-annuelles`, et un email de dépôt pour un mineur — la phrase qui s'accordait sur un NOM PROPRE (« Léa MARTIN étant mineur ») est devenue « … n'ayant pas atteint la majorité ». ⚠ **Dette assumée, arbitrée par le pilote** : les **63 captures du guide** montrent encore l'ancienne graphie (régénération écartée), de même que la documentation technique. Un **garde-fou Vitest** (`src/lib/redaction-inclusive.test.ts`, 7 tests) balaie désormais `src/` et `e2e/` à chaque exécution de la suite : il distingue la marque de genre du séparateur typographique par l'ESPACEMENT (« 12 Ko · déposé le 3 mars » reste licite et a été CONSERVÉ, décision pilote). **1010 Vitest + 289 E2E verts.** |
| 2026-08-29 | **Salve de 7 retours du personnel — DÉPLOYÉE le 2026-08-29**, dans le même bundle que le lot du point médian (`livret-bundle-2026-08-29.tar.gz`, 26 Mo, 1 402 fichiers). **AUCUN SQL** : la mise en production tient dans le remplacement du bundle ; aucune table, aucune colonne, aucune variable d'environnement, **préflight inchangé à 20/20**. ⚠⚠ **UNE FAILLE A ÉTÉ CORRIGÉE AU PASSAGE, et elle change la recette** : les pages `/admin/*` n'étaient gardées que par `requireRole()`, qui vérifie l'existence d'une session et rien d'autre — le menu masquait l'entrée, mais **l'adresse tapée à la main ouvrait l'annuaire complet** (coordonnées du personnel ET des familles) à n'importe quel rôle connecté. Découverte en traitant la demande « les comptes apprenti, tuteur et formateur ne doivent pas avoir accès à la page Utilisateurs ». Garde `requireAccesPage()` posée sur les **11** pages, avec la ressource qui pilote déjà l'entrée de menu. **À VÉRIFIER EN RECETTE, et seul un parcours réel le montre** : se connecter en apprenti, taper `/admin/utilisateurs` dans la barre d'adresse, et retomber sur le tableau de bord — le préflight ne joue que des GET anonymes, il ne verra jamais cela. ⚠ **Changement de droits à annoncer** : le formateur référent perd la création de comptes (apprenti et maître) et, avec elle, la saisie des responsables légaux d'un mineur — la page était son seul point d'entrée. ⚠ **Changement visible pour les coordos** : la page Utilisateurs ne liste plus que leurs apprentis et l'entourage de ceux-ci ; un maître ou un formateur rattaché à personne reste visible de tous, faute de quoi un compte fraîchement créé disparaîtrait avant d'avoir été affecté. Autres retours du même lot : centre d'alertes regroupé par apprenti (formateur) et par formation (coordo), « À traiter » retiré à l'admin, pastille de décompte sur « Retours du personnel », « TP » retiré des niveaux suggérés. **1031 Vitest + 297 E2E verts** (10 E2E réécrits : ils attendaient l’écran « Accès réservé » que la garde serveur remplace par une redirection). |
