...
/**
 * L'entretien tripartite (unique depuis juillet 2026) sur la trame officielle
 * GRETA (« première visite » — réunion juin 2026).
 *
 * Couvre : affichage des rubriques thématiques, points d'alerte (réponses
 * « Non »), saisie par le formateur, représentant légal et commentaires
 * individuels.
 */
...
test('l’entretien affiche la trame officielle (rubriques + bandeau d’aide)', async ({ page }) => {
  // Léa : entretien signé, rempli via la trame de démonstration.
  ...
  // Les démarches administratives (ancien format) ont disparu de l’entretien.
  await expect(page.getByText(/Démarches administratives/i)).toHaveCount(0);
});

test('les points d’alerte (réponses « Non ») sont récapitulés', async ({ page }) => {
...
test('le formateur saisit une réponse « Non » qui devient un point d’alerte', async ({
  page,
}) => {
  // On utilise un entretien frais (Sofia, non initialisé dans les fixtures).
...
test('le bloc « Représentant légal » (facultatif) est proposé au formateur', async ({
  page,
}) => {
  await page.goto('/livret/entretien');
...
test('l’entretien reste consultable par le coordinateur·rice sans saisie possible', async ({ page }) => {
...
test('3 commentaires individuels — chacun édite le sien (1ᵉʳ juillet 2026)', async ({
  page,
}) => {
  // Sofia : entretien non initialisé → le formateur l'initialise (trame vierge).
  await page.getByRole('button', { name: /Ouvrir le livret de Sofia PEREIRA/i }).click();
  await page.goto('/livret/entretien');
  await page.getByTestId('init-entretien').click();
