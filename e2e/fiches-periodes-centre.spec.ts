...
/**
 * Périodes EN CENTRE de formation (17 juin 2026, simplifiées en juillet 2026
 * — chantier référentiels/compétences #3). Le planning est géré dans la
 * modale « Planning des périodes » (section « Périodes en centre »), et
 * chaque apprenti·e dispose d'une page « Période en Centre ». La fiche
 * centre ne porte plus AUCUNE évaluation de compétence ni retour apprenti :
 * seules restent les observations de fin de période (apprenti·e bloquante
 * pour sa signature, formateur non bloquante) et les signatures à 2 parties
 * (apprenti·e + formateur).
 *
...
test('la page « Période en Centre » liste les regroupements et ouvre le détail', async ({ page }) => {
  ...
  // Détail de C1 — intitulé de la fiche + observations, SANS tableau de
  // compétences ni attitudes (fiche centre simplifiée, juillet 2026).
  ...
  await expect(page.getByText(/Suivi de la formation en centre/i)).toHaveCount(0);
  await expect(page.getByText(/Suivi de la formation en entreprise/i)).toHaveCount(0);
  await expect(page.getByTestId('attitudes-fiche')).toHaveCount(0);
...
test("au centre, l'observation bloque l'apprenti·e mais pas le formateur (juillet 2026)", async ({
  page,
}) => {
  // C2 de Léa est « en cours » : l'apprentie a signé, la formatrice non, et
  // son observation est vide — elle peut pourtant signer (non bloquante).
  await page.getByRole('link', { name: /Période en Centre/i }).click();
  await page.getByRole('link', { name: /Regroupement d.hiver/i }).click();
  await expect(page.getByRole('button', { name: /Signer en tant que Sophie/i })).toBeEnabled();
});
