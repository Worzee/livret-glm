...
| **1.5** (addendum) | **mai 2026** | Guillaume + Claude | **Extensions métier post-livraison v1.3** : ajout rôles Coordo + Admin ; refonte modulaire de l'organisation du suivi (renommée « Fiches de suivi ») ; refonte de l'entretien tripartite avec banque de questions configurable ; renommage UI « Fiches de suivi » → « Période en Entreprise » ; nouvelle section §5.7 Pronote WEB avec établissements gérés par l'admin ; matrice droits étendue à **47 ressources × 5 rôles** ; règle §10.4 verrouillage des affectations ; workflow d'import référentiels CSV + XLSX (relation N:1 + nom libre) ; **sélection par stagiaire des compétences abordées en entreprise** (décision conjointe formateur+maître, validation auto à la 3ᵉ signature entretien, R10 motivé — remplace l'ancien flag global) ; **R13 assouplie + R14 activée** (18 mai 2026 — création de période N autorisée si N-1 non signée, avertissement non bloquant listant les parties manquantes) ; polish UX (trio header, audit mobile, cohérence destructive). 8 stores Zustand persistés (au lieu de 4). **349 tests unitaires, 131 tests E2E**. |
| **1.5** vague mai 2026 | **2026-05-26** | Guillaume + Claude | **5 chantiers métier supplémentaires + équilibrage graphique** (cf. PROJECT-STATUS.md §4 « Vague mai 2026 ») : **#4** modale maître `entreprise` + `fonction` (suppression `entrepriseId`, PDF page de garde enrichi) ; **#3** suivi GRETA CFA refondu en 2 zones texte (apprenti + formateur), retrait `LigneSuiviGreta[]`, R20 formateur adaptée, 2 nouvelles ressources matrice ; **#5** import XLSX apprentis/maîtres/formateurs avec modèles téléchargeables (cellules date Excel natives, pipeline tout-ou-rien) ; **#1** planning des périodes refondu au niveau formation (`Formation.periodes[]`, cascade automatique vers tous les livrets de la promo, verrou modif/suppression si fiches signées) ; **#2** 2 entretiens tripartites par livret (`entretien1` + `entretien2`, route `/livret/entretien/:numero`, génération via événements organisation du suivi, R7 et auto-marquage sélection compétences appliqués à E1 uniquement) ; **équilibrage graphique 5 rôles** (Coordo cyan→orange foncé, Admin indigo→or foncé ; bugfix PDF aligné UI ; ~50 éléments graphiques colorisés par rôle via tokens explicites + variable CSS `--ring` dynamique + 6 utilities CSS). Bumps stores : `livret-utilisateurs` v1→v2, `livret-formations` v2→v3, `livret-donnees` v8→v10. **413 tests unitaires (+64), 131 tests E2E**. Bundle JS 148 KB gzip (+11). |

> **Note** : il n'y a pas de v1.4 publiée — la numérotation passe directement de v1.3 à v1.5 pour refléter l'ampleur des évolutions cumulées. Les chantiers de fin mai 2026 restent dans le périmètre v1.5 (extensions métier sans rupture de modèle structurelle au sens du CDC — les rôles, la matrice et les règles R1→R24 restent en place, seules certaines règles ont été assouplies ou dédoublées).

---

*Fin de l'addendum v1.5. Pour la spécification initiale, se référer au document `cahier-des-charges-livret-apprentissage-v1.3.md`. Pour l'état d'avancement courant, voir `PROJECT-STATUS.md`. Prochaine étape (auth réelle + comptes) : cf. `PROJECT-STATUS.md §12` et `TODO-etape-2.md`.*
