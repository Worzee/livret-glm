...
Ce document **complète** le cahier des charges v1.3 (`cahier-des-charges-livret-apprentissage-v1.3.md`) ; il ne le remplace pas. L'étape 1 du v1.3 a été **livrée et déployée**, puis enrichie par deux vagues d'extensions métier négociées avec le pilote et déployées dans la foulée :
...
- **Vague 2 (mai 2026)** : refontes structurantes (organisation du suivi modulaire, banque de questions d'entretien, établissements + Pronote WEB, renommages UI)
...
- [7. §6 — Matrice des droits étendue (46 ressources × 5 rôles)](#7-§6--matrice-des-droits-étendue-46-ressources--5-rôles)
...
- [12. Nouvelle annexe : flag « compétence abordée en entreprise »](#12-nouvelle-annexe--flag--compétence-abordée-en-entreprise-)
...
| Référentiels | Flag « compétence abordée en entreprise » | nouvelle annexe |
| Matrice droits | 44 → 46 ressources × 5 rôles | §6 |
...
## 7. §6 — Matrice des droits étendue (46 ressources × 5 rôles)
...
- `lib/droits.test.ts` (37 tests) vérifie que l'admin n'apparaît **jamais** dans la matrice pour les ressources pédagogiques (commentaires, niveaux, signatures, observations).
...
// Compétence — flag « abordée en entreprise »
interface Competence {
  id: string;
  code: string;
  libelle: string;
  sousFamille?: string;          // hiérarchie 3 niveaux (Bloc → Sous-famille → Compétence)
  evalueeEnEntreprise?: boolean; // défaut implicite : true
}
...
## 12. Nouvelle annexe : flag « compétence abordée en entreprise »

**Nouvelle annexe, complète §5.3 et §5.4 du v1.3.**

### Objectif

Permettre au pilote d'exclure du suivi en entreprise les compétences purement académiques (ex : connaissances théoriques évaluées uniquement en centre de formation), tout en gardant la possibilité de les évaluer côté formateur dans la grille finale.
...
interface Competence {
  // ... (existant)
  evalueeEnEntreprise?: boolean;  // défaut implicite : true (rétrocompat)
}
```

### UI

- Page `/admin/referentiels` : une **checkbox par compétence**, compteur par bloc « N/M abordée(s) en entreprise »
- Persisté dans le store référentiels (mutation `setCompetenceEvalueeEnEntreprise`)

### Cascade fonctionnelle

- `TableauTriColonnes.AjouterCompetence` (côté Période en Entreprise) n'expose que les compétences cochées
- Les lignes **déjà saisies** pour des compétences décochées **restent visibles** (cohérence historique — pas d'effacement rétroactif)
- Évaluation finale (`GrilleCompetences`) : **toutes les compétences restent affichées** (à arbitrer avec le pilote — cf. §14.A)
...
### 14.A — Évaluation finale et flag `evalueeEnEntreprise`

`GrilleCompetences` (évaluation finale) montre encore la colonne « Acquis en entreprise » pour **toutes** les compétences, indépendamment du flag. À décider : faut-il masquer/désactiver cette colonne pour les compétences `evalueeEnEntreprise === false` ? La règle actuelle ne couvre que le tableau de période en entreprise (`TableauTriColonnes`).
...
- **8 stores Zustand persistés** (au lieu de 4 en v1.3) : livret-donnees (v7), livret-role-actif, livret-apprenti-actif, livret-utilisateurs (v1), livret-formations (v2), livret-referentiels (v1), livret-banque-questions (v1), livret-etablissements (v1).
...
- **Tests TDD ciblés** sur la logique métier pure (`lib/` — 26 fichiers de tests pour 28 modules) ; les composants UI sont testés via Playwright E2E (17 specs).
...
|---|---|---|---|
| `livret-donnees` | v3 | **v7** | Reset complet à chaque bump (v3→v4 : OrganisationSuivi avec date/commentaire, v4→v5 : 6 apprenti·e·s scénarisés, v5→v6 : OrganisationSuivi modulaire, v6→v7 : entretien banque questions) |
...
| `livret-referentiels` | non existant | **v1** | Création |
